...
const appendFood = (food) => {
  ...
  <tr id="table-row-${food.id}">
    <td width="70%" id="${food.id}-${food.name}" contenteditable='true'><input class="checkbox" type="checkbox" name="drone" value="${food.id}">${food.name}</td>
    <td width="10%" id="${food.id}-${food.calories}"contenteditable='true'>${food.calories}</td>
...
$(document).ready(function() {
  $('.food-table').on('click', 'td', function() {
